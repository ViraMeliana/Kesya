...
class PerhitunganAkadController extends Controller
{
    ...
    public function calculate(StorePerhitunganAkadRequest $request, $detail)
    {
        if ($detail == "tabungan-mudharabah" ) {
            ...
            $nisbah_nasabah = 100 - $nisbah_bank;
            $result = array('last_persentase_hpp'=>round($last_persentase_hpp), 'last_dr_monthly'=>round($last_dr_monthly, 2), 'last_di_monthly'=>round($last_di_monthly, 2),
            ...
            $result = array('uang_muka'=>$uang_muka,'pembiayaan_bank'=>$pembiayaan_bank,'cost_recovery'=>intval($cost_recovery), 'margin_keuntungan'=>$margin_keuntungan,
                'harga_jual'=>intval($harga_jual),'angsuran'=>round($angsuran),'margin'=>round($margin,2),'lama_pinjam'=>$lama_pinjam,'jumlah_pinjaman'=>$jumlah_pinjaman,
                'angsuran_perbulan'=>round($angsuran_perbulan),'pembayaran_pertama'=>round($pembayaran_pertama));
        } else if ($detail == "salam" ) {
            ...
            $result = array('kebutuhan'=>$kebutuhan, 'total_pembiayaan'=>$total_pembiayaan, 'keuntungan_bank_akhir'=>$keuntungan_bank_akhir,
                'pembayaran_akhir'=>$pembayaran_akhir, 'keuntungan_bank_perbulan'=>$keuntungan_bank_perbulan, 'angsuran_perbulan'=>round($angsuran_perbulan),

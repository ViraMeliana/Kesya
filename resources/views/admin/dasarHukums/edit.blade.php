    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.dasar-hukums.update", [$dasarHukum->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('ringkasan') ? 'is-invalid' : '' }}" name="ringkasan" id="ringkasan" required>{{ old('ringkasan', $dasarHukum->ringkasan) }}</textarea>
                @if($errors->has('ringkasan'))
                    <div class="invalid-feedback">
                        {{ $errors->first('ringkasan') }}
                    </div>
                @endif
                <span class="help-block">{{ trans('cruds.dasarHukum.fields.detail_helper') }}</span>
            </div>
            <div class="form-group">

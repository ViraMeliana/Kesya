    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.dasar-hukums.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('kategori_dasar_hukum') ? 'is-invalid' : '' }}" name="kategori_dasar_hukum_id" id="kategori_dasar_hukum_id" required>
                    @foreach($kategori_dasar_hukums as $id => $entry)
                        <option value="{{ $id }}" {{ old('kategori_dasar_hukum_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('nama_hukum') ? 'is-invalid' : '' }}" type="text" name="nama_hukum" id="nama_hukum" value="{{ old('nama_hukum', '') }}" required>
